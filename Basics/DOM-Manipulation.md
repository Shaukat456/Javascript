...
titleElement.innerHTML =
  "<strong>Welcome</strong> to Our <em>Amazing</em> Store!";
...
function updateUserDashboard(user) {
  ...
  statusIndicator.className = user.isOnline
    ? "status online"
    : "status offline";
}
...
function createProductCard(product) {
  ...
  card.innerHTML = `
        <div class="product-image">
            ...
            <div class="product-badge ${product.isNew ? "new" : ""}">${
    product.isNew ? "NEW" : ""
  }</div>
        </div>
...
class TodoApp {
  ...
  renderTodos() {
    ...
    filteredTodos.forEach((todo) => {
      ...
      editBtn.addEventListener("click", () =>
        this.enableEdit(todo.id, textSpan)
      );
  ...
  updateStats() {
    ...
    document.getElementById("todo-count").textContent = `${activeCount} item${
      activeCount !== 1 ? "s" : ""
    } left`;
